...
func main() {
	...
	// --- diagnose-vc command ---
	var vcFightID int
	var vcPlayerName string

	diagnoseVCCmd := &cobra.Command{
		Use:   "diagnose-vc <report_code>",
		Short: "Test whether Vigorous Creepers is additive or multiplicative with mastery",
		Args:  cobra.ExactArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			runDiagnoseVC(args[0], vcFightID, vcPlayerName)
		},
	}
	diagnoseVCCmd.Flags().IntVar(&vcFightID, "fight", 0, "Fight ID (interactive if omitted)")
	diagnoseVCCmd.Flags().StringVar(&vcPlayerName, "player", "", "Player name (interactive if omitted)")
	rootCmd.AddCommand(diagnoseVCCmd)
...
func runDiagnoseVC(reportCode string, fightID int, playerName string) {
	client := newClient()
	...
	diag := analysis.NewVCDiagnostic(config.Mastery.DRTable)
	result := diag.Run(events)
	fmt.Print(result)
}
